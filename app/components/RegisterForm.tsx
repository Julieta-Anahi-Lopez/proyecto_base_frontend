...
export default function RegisterForm({ onCancel }: { onCancel: () => void }) {
  ...
  const validateForm = (): string | null => {
    ...
    if (!formData.nombre || !formData.clave || !formData.claveConfirm || !formData.e_mail || !formData.nrodoc || !formData.cativa) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-8 max-w-md w-full">
      ...
      <div>
        ...
        {/* Categoria IVA */}
        <div>
          <label htmlFor="cativa" className="block text-sm font-medium text-gray-700 mb-1">
            Categoría frente al IVA *
          </label>
          <select
            id="cativa"
            name="cativa"
            value={formData.cativa}
            onChange={handleChange}
            required
            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-gray-700"
          >
            ...
            <option value='7'>Monotributista</option>
